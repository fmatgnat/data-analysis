...
ref_file      = 'Mercury_Na_804_reference_image.fits'                     ; a reference image of Mercury (combined from "good" Mercury data) -- to be used for finding alignments below
...
if part eq 1 or part eq 99 then begin ;Find the centroids by cross-correlation with the last image
    ...
    int       = sxpar(header, 'EXPOSURE')                                 ; integration time for individual frames
    dint      = sxpar(dark_header, 'EXPOSURE')                            ; integration time for dark
    fint      = sxpar(flat_header, 'EXPOSURE')                            ; integration time for flat
    ...
    iflat     = iflat[ims(0):ims(1),ims(2):ims(3)]                        ; extract and normalize to counts/sec
;    flat      = ( iflat - dark ) / max( iflat - dark )
...
;        ireference = MRDFITS(Mercury_dir + ref_file, ref_header, /unsigned, /silent)
    ...
    ;xslit_pixels = where( (total(flat,2) - yfit) ge stddev_yfit )        ; find x pixels more than 1 stddev below mean
...
;        iframe = (reform(imaging_cube[*,*,i])/int - dark) / flat          ; "raw" imaging frame
...
;        iframe2 = iframe
        ...
        ; Now replace slit pixels with randomizied nearby average
;        Y_Mercury = where( normalize(total(iframe,1,/nan) - min(totaL(iframe,1,/nan))) gt 0.3 ) ; approximate Y pixels of Mercury's disk
;        top   = iframe(*,max(Y_Mercury)+10:*)                             ; extract upper half of image array
;        bot   = iframe(*,0:min(Y_Mercury)-10)                             ; extract bottom half of image array
;        islit_top = top(xslit-slit_width-20:xslit-slit_width-1,*)         ; 20 nearby columns before slit
;        fslit_top = top(xslit+slit_width+1:xslit+slit_width+20,*)         ; 20 nearby columns after slit
;        islit_bot = bot(xslit-slit_width-20:xslit-slit_width-1,*)         ; 20 nearby columns before slit
;        fslit_bot = bot(xslit+slit_width+1:xslit+slit_width+20,*)         ; 20 nearby columns after slit
;        for j = 0, n_elements(islit_top(0,*))-1 do begin                  ; assign random values to slit based on nearby pixels (TOP)
;          slit = median(islit_top(*,j) + fslit_top(*,j))/2. + stddev(islit_top(*,j) + fslit_top(*,j))*(randomu(j^2.34,slit_width*2+1)-0.5)
;          iframe2(xslit-slit_width:xslit+slit_width,max(Y_mercury)+10+j) = slit
;        endfor ;j
;        for j = min(Y_Mercury)-10, max(Y_Mercury)+10 do begin             ; assign random values to slit based on nearby pixels (MERCURY)
;          slit = median(iframe(xslit-slit_width-20:xslit-slit_width-1,j) + iframe(xslit-slit_width+1:xslit+slit_width+20,j))/2. $
;               + stddev(iframe(xslit-slit_width-20:xslit-slit_width-1,j) + iframe(xslit+slit_width+1:xslit+slit_width+20,j)) $
;               * (randomu(j^2.34,slit_width*2+1)-0.5)
;          iframe2(xslit-slit_width:xslit+slit_width,j) = slit
;        endfor ;j
;        for j = 0, n_elements(islit_bot(0,*))-1 do begin                  ; assign random values to slit based on nearby pixels (BOTTOM)
;          slit = median(islit_bot(*,j) + fslit_bot(*,j))/2. + stddev(islit_bot(*,j) + fslit_bot(*,j))*(randomu(j^2.34,slit_width*2+1)-0.5)
;          iframe2(xslit-slit_width:xslit+slit_width,j) = slit
;        endfor ;j
        
;        oframe = iframe
;        fill_missing, iframe, !values.F_Nan, 1                            ; interpolate over slit
;        fill_missing_rand, oframe, !values.F_Nan, 1
;        wset, 0
;        cgimage, iframe2(200:280,300:*), /axes
;        wset, 1
;        cgimage, iframe(200:280,300:*), /axes
;        wset, 2
;        cgplot, total(iframe2(*,250:300),2)
;        cgplot, total(iframe(*,250:300),2), /overplot, col='red'
;        stop
;        wset, 0
;        cgimage, bytscl(oframe(200:280,100:220),0,55000), /axes, title='oframe'
;        wset, 1
;        cgimage, bytscl(iframe(200:280,100:220),0,55000), /axes, title='iframe'
;        wset, 2
;        cgimage, bytscl(iframe2(200:280,100:220),0,55000), /axes, title='iframe2'
;        stop
...
if part eq 2 or part eq 99 then begin 
    ...
    iDark = MRDFITS(dark_dir + dark_file, 0, dark_header, /fscale, /silent )   ; read in dark
    iflat = MRDFITS(flat_dir + flat_file, 0, flat_header, /fscale )            ; read flat, extract imaging portion, scale
    int       = sxpar(header2, 'EXPOSURE')                                ; integration time for frames
    dint      = sxpar(dark_header, 'EXPOSURE')                            ; integration time for dark
    fint      = sxpar(flat_header, 'EXPOSURE')                            ; integration time for flat
...
;        spectra_sky_cube = MRDFITS(outdir + 'spectra_sky_cube.fits', header, /fscale, /silent ) ; not working for some strange reason... using readfits instead for now
        ...
        D1_x            = (where(frame_spectrum eq min(frame_spectrum)))[0]    ; x pixel for D1 line
        ...
        D2_x            = (where(frame_temp eq min(frame_temp)))[0]       ; x pixel for D2 line
        if D1_x lt D2_x then Na_D_rngs = [D1_x-Na_D_rng, D1_x+Na_D_rng, D2_x-Na_D_rng, D2_x+Na_D_rng] $
        ...
        for i = 0, s[2]-1 do begin                                        ; LOOP over each frame in the series
            ...
            wset, 0
;            cgimage, bytscl(frame, 0, max(frame)), /axes                         ; plot the original spectral frame
            ...
            wset, 1
;            if do_smooth then cgimage, bytscl(scaled_reference, 0, max(scaled_reference)), /axes, title='smoothed and scaled by ' + strfix(best_fac) $
;                         else cgimage, bytscl(scaled_reference, 0, max(scaled_reference)), /axes, title='scaled by ' + strfix(best_fac)
            ...
            wset, 2
;            cgimage, bytscl(Just_exosphere, 0, max(Just_exosphere)), /axes                ; plot the Mercury Na emission after subtracting disk
...
;    home           = [ (where(total(total(aligned_imaging_cube,3),2) eq max(total(total(aligned_imaging_cube,3),2))))[0], $
;                        (where(total(total(aligned_imaging_cube,3),1) eq max(total(total(aligned_imaging_cube,3),1))))[0] ]
    
;    big_cube_spec  = fltarr(s[0], s[1], s[2]) + !Values.F_NAN             ; initialize cube of spectral image data
...
;    big_frame_total = fltarr(s[0], 2*s[1])                                ; combined imaging and spectral frame
...
;    ycorrection    = (where(normalize(total(total(aligned_spectra_cube,3),1) - min(total(total(aligned_spectra_cube,3),1))) gt 0.05))[0] - $
;                     (where(normalize(total(brightness,1,/nan)) gt 0.05))[0]  ; this correction accounts for the resized brightness array (ie the Mercury Na emission begins in a different place)
    ymax_spec      = fix(mean( where(normalize(total(rotated_brightness,1,/nan)) ge 0.05) ))
;    ymax_spec      = (where(total(total(aligned_spectra_cube,3),1) eq max(total(total(aligned_spectra_cube,3),1))))[0] - ycorrection
    ...
    ;ytot_img       = normalize(total(total(aligned_imaging_cube,3),1) - min(total(total(aligned_imaging_cube,3),1)))
...
;        empty_frame[home+shift_array[i,0]-1:home+shift_array[i,0]+1,*] = [brightness[i, *], brightness[i, *], brightness[i, *]] ;Make it bigger?
...
;        big_frame[*, s[1]:*] = rotate(empty_frame, 7)                     ; rotate image if applicable (note 7 = transposed and rotated ccw 270 deg)
        big_frame[*, s[1]:*] = empty_frame
;        big_frame[*, 0:s[1]-1] = reform(aligned_imaging_cube[*,*,i])
        big_frame[*, 0:s[1]-1] = reform(aligned_imaging_cube[*,*,i]) * max(empty_frame,/nan) / max(reform(aligned_imaging_cube[*,*,i]),/nan)  ; the max ratios scale the bottom of big_frame based on the relative values in imaging and spectral channels
        big_frame_total = big_frame_total + big_frame
;        print, i, max(empty_frame,/nan) / max(reform(aligned_imaging_cube[*,*,i]),/nan), max(big_frame(*,s[1]:*),/nan), max(big_frame(*,0:s[1]-1),/nan)

        ; Create frames for "Mercury scan" movie -- this shows the image and extracted Na spectrum frame-by-frame
        wset, 0
        cgloadct, ct, /silent
;        big_frame_scaled = big_frame / (total(big_frame,/nan)/7.427e7)  ; LM:commented out for now as we've scaled big_frame above when inserting aligned_imaging_cube
...
;        spec_summed = total(spec_frame,2)                                 ; by summing rows we can find the columns corresponding to the slit location
...
;        spec_movie = bytscl(spec_movie_temp, max_spec*0.05, max_spec*0.8)
        print, i, max(spec_movie_temp,/nan)
        spec_movie = bytscl(spec_movie_temp, 0., max_spec)
;        spec_movie = spec_movie_temp
;        stop
...
;        cgcontour, normalize(img_cube_cut), /onimage, levels=[contour_outline], label=0, col='red'
        ...
        wset, 2
;        if do_smooth then img_count_cut = smooth(img_count(img_extraction(0):img_extraction(1),img_extraction(2):img_extraction(3)),smooth_width,/edge_truncate) $
...
;        cgimage, smooth(big_frame_total_top,[2,smooth_width],/nan)
...
;        cgimage, bytscl(big_frame_total_bottom, 0, big_frame_max*ig), missing_value=!Values.F_NaN
...
;   cgcontour, normalize(img_cube_cut), /onimage, levels=[contour_outline], label=0
   cgcontour, normalize(reference), /onimage, levels=[contour_outline], label=0
   cgcolorbar, /vertical, charsize=1.e-3, position=[0.9,0.15,0.95,0.85]
...
;   cgcontour, normalize(img_cube_cut), /onimage, levels=[contour_outline]
   cgcontour, normalize(reference), /onimage, levels=[contour_outline], label=0
